...
# These replace the old JSON prompt engineering — Ollama handles schema enforcement
TOOL_SCHEMAS = [
...
async def call_ollama(model: str, messages: list, thinking: bool = False,
    ...
    """
    Call Ollama with optional native tool calling.
    Returns (final_text, tool_results_summary).

    Tool loop:
    1. Send messages + tool schemas to model
    2. If model returns tool_calls → execute them → append results → call again
    3. Repeat up to MAX_TOOL_LOOPS times
    4. Return final text response
    """
    msgs = list(messages)

    # Suppress thinking for tool model and simple chat
    ...
    for loop in range(MAX_TOOL_LOOPS + 1):
        ...
        # Attach tool schemas only when using tool model
        # 4B models are unreliable with >8 tools — use core subset
        if use_tools:
            payload["tools"] = CORE_TOOL_SCHEMAS

        # sock_read=None means no read timeout — required for 14B on CPU
        # Connection must establish within 60s, but inference can take as long as needed
        ...
        # No tool calls or loop protection exhausted → return text
        ...
        for tc in tool_calls:
            ...
            # arguments may be a string (some Ollama versions)
...
# ==================== SYSTEM PROMPTS ====================
# Minimal — do NOT add JSON examples or tool listings here.
# Tool schemas are injected via the tools= parameter; duplicating them confuses small models.
SYSTEM_PROMPT = {
...
# Separate minimal prompt for the router pass (no tools attached here)
...
async def chat_completion(request: Request, body: ChatRequest):
    try:
        ...
        if explicit_model:
            ...
            # ── Two-pass router: lightweight classifier first ──────────────
            # Use the fast 4B model just to classify intent (TOOL vs CHAT).
            # This is more reliable than keyword matching and avoids overloading
            # the main model with routing decisions.
            try:
                router_msgs = [
        ...
        # If the model returned empty content but tools ran, summarise
